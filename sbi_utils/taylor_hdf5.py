...
class Taylor_Dataset(Dataset):
	def __init__(self,phase='train',image_size=224,n_frames=8):
		
		assert phase in ['train','val','test']
		
		image_list,label_list=init_ff_taylor(phase,'frame',n_frames=n_frames)
	...
	def __getitem__(self,idx):
		...
		while flag:
			try:
				...
				landmark=np.load(filename.replace('-taylor', '++').replace('.png','.npy').replace('/frames/',self.path_lm))[0]
				bbox_lm=np.array([landmark[:,0].min(),landmark[:,1].min(),landmark[:,0].max(),landmark[:,1].max()])
				bboxes=np.load(filename.replace('-taylor', '++').replace('.png','.npy').replace('/frames/','/retina/'))[:2]
				iou_max=-1
				...
				if idx < 20:
					cv2.imwrite('output/visualization/blend_taylor/debug_img_f' + str(idx) + '.jpg', cv2.cvtColor(img_f, cv2.COLOR_BGR2RGB))
					cv2.imwrite('output/visualization/blend_taylor/debug_img_r' + str(idx) + '.jpg', cv2.cvtColor(img_r, cv2.COLOR_BGR2RGB))
...
class Taylor_Video_HDF5_Dataset(Dataset):
	def __init__(self,phase='train',image_size=224,n_frames=8):
		...
		while flag:
			try:
				...
				for frame_idx in range(self.n_frames):
					...
					img = np.array(Image.fromarray(cv2.cvtColor(im,cv2.COLOR_BGR2RGB)))

					landmark=self.data[filename.replace('.png','.npy').replace('/frames/',self.path_lm)][0]
					bbox_lm=np.array([landmark[:,0].min(),landmark[:,1].min(),landmark[:,0].max(),landmark[:,1].max()])
					bboxes=self.data[filename.replace('.png','.npy').replace('/frames/','/retina/')][:2]
					iou_max=-1
...
class Video_Dataset(Dataset):
	def __init__(self,phase='train',image_size=224,n_frames=8):
		...
		image_list,label_list=init_ff_video(dataset='all',phase=phase)
